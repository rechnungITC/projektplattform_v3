...
**Erstellt:** 2026-06-15 · **Quelle:** Continuous Improvement Agent (CIA-Review) · **Status:** Decision-Input + Readiness-Update 2026-06-23
...
> **Readiness-Guide:** Die operative Sicht "was fehlt noch, damit M&A-Projekte durchgeführt werden können?" steht in [`docs/ma-project-execution-readiness.md`](ma-project-execution-readiness.md). Dieses Sequencing-Dokument erklärt die Reihenfolge und Reuse-Logik; der Readiness-Guide übersetzt sie in konkrete Produktfähigkeit.
...
**Update 2026-06-23:** Die Domänen-ADR ist accepted, PROJ-100a ist approved, und PROJ-94 ist QA PASS mit offenem PR #168. Damit ist "M&A-Projekt anlegen" fast produktiv greifbar, aber M&A-Projekte sind fachlich noch nicht durchführbar. Dafür fehlen mindestens PROJ-100b, PROJ-95, PROJ-97, PROJ-99/128/129 und der DD-Kern PROJ-112-116.
...
**Readiness nach PROJ-94:** PROJ-94 deckt nur den ersten Schritt "Deal anlegen" ab. Der kuerzeste noch offene Pfad zum DD-Pilot ist:

```
100b (Need-to-Know bedienbar machen) -> 95 (Phasen/Gates) -> 97 (RACI)
  -> 99/128/129 (externe Berater, NDA, Klassifikation)
  -> 112 (DD-Streams) -> 113 (Q&A) -> 114 (Findings)
  -> 108 (Red-Flags) -> 110/111 (Gate + Decision)
  -> 116 (DD-/Red-Flag-Report)
```

## 4. Architektur-Forks (VOR /architecture locken)

**Fork 1 — M&A als `project_type` vs. eigenes Modul.** Empfehlung: **`project_type='ma'` mit Label "M&A-Projekt" über PROJ-6 Rule-Engine + Method-Catalog** (10 Phasen + 9 Stage-Gates als Methode), M&A-Objekte als Extensions via `v3-code-extension-pattern.md`. Begründung: respektiert „Shared-core"; ERP/Bau/Software laufen schon so. Eigenes Modul = Silo.
...
- **Release 0 (Foundation-Lock, zwingend):** Domänen-ADR (Forks 1–5) + PROJ-6 `project_type='ma'` + M&A-Method-Catalog (Phasen+Gates) + 100 Need-to-Know-RLS.
