...
| PROJ-91 | P1 | AI Backlog Grounding in Project Intent — Generierung sieht das Wizard-„Vorhaben" (`projects.description`) nicht → KI validiert Kickoff-Inhalte nicht gegen das Projektziel. Fix: Vorhaben in Kontext+Prompt (Grounding) + `relevance`-Flag (`on_goal`/`off_goal`) + „≠ Ziel"-Badge. „Grounding + Relevanz-Flag" (kein Block). | Architected |
