...
| PROJ-45 | P2 | Construction extension: Gewerke-Katalog + Bauabschnitte als zweite Gliederungsachse (α), Mängelmanagement mit Zwei-Akteur-Prüfung und Mängelanzeige (β), Abnahmen mit Abnahmeprotokoll, Vorbehalten und Gewährleistungsfrist (γ), bauspezifische Terminsignale (δ), Fotodokumentation (ε). Strikt Core-Reuse: `project_type='construction'`, keine Parallelmodule; Gewerke werden gespiegelt statt `workstreams` zu generalisieren (CIA 2026-08-13). | Deployed (α + β + γ + δ, Scope `alpha`); δ live 2026-08-21 (Tag `v2.73.0-PROJ-45-delta`) — damit ist AC-45β.18 erledigt und **ε (Fotodokumentation) der einzige verbleibende Grund für `alpha` statt `full`**; γ live 2026-08-20 (Tag `v2.70.0-PROJ-45-gamma`); δ architected 2026-08-20 (Terminsignale, 24 AC + 14 Härtungskriterien, enthält AC-45β.18 — Gantt bleibt per Nutzer-Entscheid unberührt; eigene Fläche plus optionaler Bau-Block im PROJ-21-Status-Report, kein CIA-Pass), bereit für `/backend`; ε **architected 2026-08-24** (Fotodokumentation: 23 AC + 12 Härtungskriterien, 8 Locks L31–L38, alle 7 Architekturfragen beantwortet; der CIA-Review zu Q-ε1 verschiebt die Frage von Können auf **Lizenz**: 1166 Pakete im Prod-Baum, keine einzige GPL-only, während die HEIC-Dekoder libde265 bündeln — Nutzer-Entscheid „erst mit einer echten Datei messen, dann entscheiden", also **keine neue Abhängigkeit im Entwurf**), noch nicht gebaut und damit der **einzige** verbleibende Grund für Scope `alpha` |
...
| PROJ-92 | P1 | Azure OpenAI Provider (Class-1/2) — fünfter Provider-Typ (tenant-eigene Azure-Ressource, EU-Region-Allowlist, openai-compatible Factory ohne neues Dep). Invariante #3 unberührt. | Deployed |
| PROJ-93 | P1 | Trusted-EU-Processor — kontrollierte, opt-in Class-3-Freigabe NUR für DPA-attestiertes Azure OpenAI im EU-Kunden-Tenant; DPA-konditionaler DB-CHECK, ADR + Invariante-#3-Präzisierung. Ohne Attest: Ollama-only unverändert. | Deployed |
