...
| PROJ-25b | P2 | Backlog↔Sprint DnD via `@dnd-kit/core` + a11y-Polish (aria-live, Keyboard-DnD, Escape-Cancel) + Multi-Select (Ctrl/Shift + Bulk-API) + Perf-Benchmark (60fps). Schließt PROJ-25-Deferred-Items D-1 bis D-5. | Deployed |
| PROJ-28 | P2 | Method-aware Project-Room Navigation: Sidebar-Labels + URL-Slugs (Wasserfall: /arbeitspakete, /phasen; Scrum: /releases) per Methode. Schließt PROJ-26 deferred-frontend (L1). | Deployed |
