...
| [soft-delete-enforcement-scope.md](soft-delete-enforcement-scope.md) | Der Papierkorb (`projects.is_deleted`) ist eine Grenze der Anwendungsschicht: durchgesetzt **einmal** in `requireProjectAccess` (404), bewusst **nicht** wiederholt in den 11 projektbezogenen Auswertungsfunktionen und nicht in der RLS der Kindtabellen. |
...
| [skills-data-model.md](skills-data-model.md) | **V3-original.** PROJ-76: Skill authoring is form-first — structured metadata (JSON) is the source of truth, the `.md` string is serialised server-side via `js-yaml.dump()` (never parsed back). No `gray-matter`, no Markdown editor/renderer (zero new transitive deps). Tag vocabularies bind to real `PROJECT_METHODS`/`ProjectType` constants; empty = all. |
| [skill-versioning.md](skill-versioning.md) | **V3-original.** PROJ-76: immutable `skill_versions` rows, at most one `active` per skill (partial-unique + `current_version_id`), activate/rollback via `SECURITY DEFINER` RPCs (mirrors PROJ-20 decisions immutability + state-machine convention); rollback creates a new version, never mutates history; both tables opt into PROJ-10 audit. **PROJ-77-α amendment:** drafts become editable in place (status-gated trigger relaxation, `updated_at`/`If-Match`), active/archived stay frozen, rollback unchanged, one-open-draft app-layer. |
| [skill-allowed-actions.md](skill-allowed-actions.md) | **V3-original.** PROJ-77-α: a skill version carries an optional `allowed_actions` allow-list (fixed V1 enum), stored + validated in PROJ-77; enforcement (403 + `skill.action_denied` audit) is owned by PROJ-82/83 and is **fail-closed** (empty/absent ⇒ no mutating action). |
