...
type EditMilestoneValues = z.infer<typeof editMilestoneSchema>
...
export function EditMilestoneDialog({
  ...
  const defaultValues = React.useMemo<EditMilestoneValues>(
    () => ({
      phase_id: milestone.phase_id,
      name: milestone.name,
      description: milestone.description ?? "",
      target_date: parseIsoDate(milestone.target_date),
    }),
    [milestone]
  )

  const form = useForm<EditMilestoneValues>({
  ...
  const onSubmit = async (values: EditMilestoneValues) => {
    setSubmitting(true)
    try {
      const payload = {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Stammdaten ändern. Status wird über die Statusaktion gepflegt.
          </DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form
