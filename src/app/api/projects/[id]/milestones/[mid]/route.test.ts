import { beforeEach, describe, expect, it, vi } from "vitest"

// PROJ-19 — single-milestone PATCH drift-detection test.

const getUserMock = vi.fn()

const milestoneChain = {
  select: vi.fn().mockReturnThis(),
  update: vi.fn().mockReturnThis(),
  eq: vi.fn().mockReturnThis(),
  maybeSingle: vi.fn(),
  single: vi.fn(),
}

const fromMock = vi.fn((table: string) => {
  if (table === "milestones") return milestoneChain
  throw new Error(`unexpected table ${table}`)
})

vi.mock("@/lib/supabase/server", () => ({
  createClient: vi.fn(async () => ({
    auth: { getUser: getUserMock },
    from: fromMock,
  })),
}))

import { PATCH } from "./route"

const PROJECT_ID = "11111111-1111-4111-8111-111111111111"
const MILESTONE_ID = "44444444-4444-4444-8444-444444444444"
const USER_ID = "33333333-3333-4333-8333-333333333333"

beforeEach(() => {
  vi.clearAllMocks()
  milestoneChain.select.mockReturnValue(milestoneChain)
  milestoneChain.update.mockReturnValue(milestoneChain)
  milestoneChain.eq.mockReturnValue(milestoneChain)
})

// ---------------------------------------------------------------------------
// Schema-vs-DB-Payload Drift Detection (PATCH)
// ---------------------------------------------------------------------------
describe("PATCH /api/projects/[id]/milestones/[mid] — schema/DB-payload drift", () => {
  it("forwards every Zod-schema field to the DB update payload", async () => {
    const { milestonePatchSchema } = await import("../_schema")
    getUserMock.mockResolvedValue({ data: { user: { id: USER_ID } } })

    // patchSchema is wrapped in .refine() — unwrap via _def.schema.shape.
    const inner =
      "shape" in milestonePatchSchema
        ? (milestonePatchSchema as unknown as {
            shape: Record<string, unknown>
          })
        : (
            milestonePatchSchema as unknown as {
              _def: { schema: { shape: Record<string, unknown> } }
            }
          )._def.schema
    const schemaKeys = Object.keys(
      (inner as { shape: Record<string, unknown> }).shape
    )

    const PHASE_ID = "77777777-7777-4777-8777-777777777777"
    const kitchenSink: Record<string, unknown> = {
      phase_id: PHASE_ID,
      name: "Drift-Test Milestone",
      description: "Updated description.",
      target_date: "2026-12-31",
      actual_date: "2026-12-30",
      status: "achieved",
    }

    for (const key of schemaKeys) {
      expect(kitchenSink, `kitchen sink missing key '${key}'`).toHaveProperty(
        key
      )
    }

    milestoneChain.single.mockResolvedValue({
      data: { id: MILESTONE_ID, ...kitchenSink },
      error: null,
    })

    const res = await PATCH(
      new Request("http://localhost/x", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(kitchenSink),
      }),
      { params: Promise.resolve({ id: PROJECT_ID, mid: MILESTONE_ID }) }
    )
    expect(res.status).toBe(200)

    const arg = milestoneChain.update.mock.calls[0]?.[0] as Record<
      string,
      unknown
    >
    expect(arg, "update was not called").toBeTruthy()

    for (const key of schemaKeys) {
      const expected = kitchenSink[key]
      const actual = arg[key]
      // target_date / actual_date are YYYY-MM-DD — not trimmed.
      if (
        typeof expected === "string" &&
        key !== "target_date" &&
        key !== "actual_date"
      ) {
        expect(actual, `field '${key}' was dropped before reaching DB`).toBe(
          expected.trim() || null
        )
      } else {
        expect(actual, `field '${key}' was dropped before reaching DB`).toBe(
          expected
        )
      }
    }
  })
})
