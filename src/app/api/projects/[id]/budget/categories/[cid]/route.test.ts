...
// PROJ-22 — single budget category PATCH drift-detection test.
...
beforeEach(() => {
  ...
  categoriesChain.update.mockReturnValue(categoriesChain)
...
// ---------------------------------------------------------------------------
describe("PATCH /api/projects/[id]/budget/categories/[cid] — schema/DB-payload drift", () => {
  it("forwards every Zod-schema field to the DB update payload", async () => {
    const { budgetCategoryPatchSchema } = await import("../_schema")
    getUserMock.mockResolvedValue({ data: { user: { id: USER_ID } } })

    // patchSchema is wrapped in .refine() — unwrap via _def.schema.shape.
    const inner =
      "shape" in budgetCategoryPatchSchema
        ? (budgetCategoryPatchSchema as unknown as {
    ...
    categoriesChain.maybeSingle.mockResolvedValue({
      data: { id: CATEGORY_ID, ...kitchenSink },
      error: null,
    })
    ...
    const res = await PATCH(
      ...
      { params: Promise.resolve({ id: PROJECT_ID, cid: CATEGORY_ID }) }
    )
    expect(res.status).toBe(200)

    const arg = categoriesChain.update.mock.calls[0]?.[0] as Record<
      string,
      unknown
    >
