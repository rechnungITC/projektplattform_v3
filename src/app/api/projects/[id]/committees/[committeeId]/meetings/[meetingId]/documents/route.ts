...
import { addDocumentSchema } from "../../_schema"

// PROJ-117 — add a pre-read / minutes-attachment LINK via add_meeting_document RPC.
export async function POST(
  ...
  const { data, error } = await supabase.rpc("add_meeting_document", {
    p_meeting_id: meetingId,
    p_label: parsed.data.label,
    p_url: parsed.data.url,
    p_kind: parsed.data.kind ?? "pre_read",
  })
  if (error) {
