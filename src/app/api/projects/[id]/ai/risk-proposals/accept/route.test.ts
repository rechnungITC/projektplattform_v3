/**
 * PROJ-89 — vitest for the risk-proposals bulk-accept route.
 *
 ...
 *   - happy path → 200 with id arrays incl. created_risk_ids +
 *     linked_risk_ids
 *   - RPC duplicate_target_not_found (23514) → 400
...
function makePost(body: unknown): Request {
  return new Request(
    `http://localhost/api/projects/${PROJECT_ID}/ai/risk-proposals/accept`,
    {
      method: "POST",
...
describe("POST /api/projects/[id]/ai/risk-proposals/accept", () => {
  ...
  it("returns 200 + id arrays on RPC success", async () => {
    setupEditorAccess()
    const acceptedAt = "2026-06-11T12:00:00Z"
    rpcMock.mockResolvedValue({
      data: [
        {
          accepted_suggestion_ids: [SUG_1, SUG_2],
          created_risk_ids: ["risk-1"],
          linked_risk_ids: ["risk-existing"],
    ...
    expect(body.created_risk_ids).toEqual(["risk-1"])
    expect(body.linked_risk_ids).toEqual(["risk-existing"])
    expect(body.accepted_at).toBe(acceptedAt)
    expect(rpcMock).toHaveBeenCalledWith("accept_risk_proposals_bulk", {
      p_project_id: PROJECT_ID,
      p_suggestion_ids: [SUG_1, SUG_2],
    })
  })

  it("returns 400 on RPC duplicate_target_not_found (23514)", async () => {
    setupEditorAccess()
    rpcMock.mockResolvedValue({
      data: null,
      error: { code: "23514", message: "duplicate_target_not_found" },
    })
    const res = await POST(makePost({ suggestionIds: [SUG_1] }), {
      params: Promise.resolve({ id: PROJECT_ID }),
    })
