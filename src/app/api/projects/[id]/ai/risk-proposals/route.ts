/**
 * PROJ-89 — risk proposals from a kickoff context source.
 *
 *   POST /api/projects/[id]/ai/risk-proposals
 *     Body: { contextSourceId: uuid, count?: 1–30 (default 10) }
 *     → triggers `invokeRiskProposalsGeneration` and returns the
 *       run result + new suggestion ids.
 *
 *   GET  /api/projects/[id]/ai/risk-proposals?status=draft|accepted|rejected
 *     → lists `ki_suggestions` rows with
 *       `purpose='proposal_risks_from_context'`, newest first.
 *
 * Auth: editor+ for POST, member for GET. Classification is CONTENT-BASED
 * (AC-89.2): clean Class-2 documents route to the tenant cloud provider;
 * PII markers in the excerpt/Vorhaben or a stamped Class-3 source clamp
 * to the local provider set via the standard resolver (invariant #3).
 */

import { NextResponse } from "next/server"
import { z } from "zod"

import { collectRiskProposalsAutoContext } from "@/lib/ai/auto-context"
import { invokeRiskProposalsGeneration } from "@/lib/ai/router"
...
export async function GET(request: Request, ctx: Ctx) {
  ...
  let query = supabase
    ...
    .eq("purpose", "proposal_risks_from_context")
...
export async function POST(request: Request, ctx: Ctx) {
  ...
  let context
  try {
    context = await collectRiskProposalsAutoContext(
      supabase,
      projectId,
  ...
  const result = await invokeRiskProposalsGeneration({
    supabase,
