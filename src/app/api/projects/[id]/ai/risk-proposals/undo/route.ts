/**
 * PROJ-89 — undo a risk-proposals bulk-accept (30-second window).
 *
 *   POST /api/projects/[id]/ai/risk-proposals/undo
 *     Body: { suggestionIds: uuid[] }
 *
 * Via the `accept_risk_proposals_undo` RPC: deletes ONLY the risks this
 * accept created, never pre-existing `risk_link` targets; cleans
 * `ki_provenance` (H-2 lesson) and resets suggestions to draft through
 * the controlled immutability bypass. Same-actor + 30-s window enforced
 * in the RPC.
 */
...
export async function POST(request: Request, ctx: Ctx) {
  ...
  const { data, error } = await supabase.rpc("accept_risk_proposals_undo", {
  ...
  return NextResponse.json(
    {
      ...
      reverted_risk_ids: row.reverted_risk_ids ?? [],
    },
    { status: 200 },
  )
}
