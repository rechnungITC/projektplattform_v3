/**
 * PROJ-70-β — accept a batch of proposal_from_context suggestions.
 *
 *   POST /api/projects/[id]/ai/proposal-from-context/accept
 *     Body: {
 *       suggestionIds: uuid[],
 *       methodValidationStrict?: boolean (default true)
 *     }
 *
 * Server-side topological-sort + atomic transaction via the
 * `accept_proposal_from_context_bulk` RPC. Returns the
 * `accepted_suggestion_ids[]`, `created_work_item_ids[]`, and
 * `accepted_at` timestamp the FE needs for the 30-s Undo button.
 *
...
const postBodySchema = z.object({
  ...
  methodValidationStrict: z.boolean().optional().default(true),
})
...
export async function POST(request: Request, ctx: Ctx) {
  ...
  const { data, error } = await supabase.rpc(
    "accept_proposal_from_context_bulk",
    {
      ...
      p_method_validation_strict: parsed.data.methodValidationStrict,
    },
  )

  if (error) {
    // Map known RPC error codes to user-facing API errors.
    if (error.code === "P0002") {
  ...
  // `data` is the array of returned rows from the table-returning RPC.
  // Bulk-accept returns exactly one row.
  ...
  return NextResponse.json(
    {
      ...
      created_work_item_ids: row.created_work_item_ids ?? [],
