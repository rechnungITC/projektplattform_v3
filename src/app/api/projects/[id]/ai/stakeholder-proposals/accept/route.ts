/**
 * PROJ-88 — accept a batch of proposal_stakeholders_from_context suggestions.
 *
 *   POST /api/projects/[id]/ai/stakeholder-proposals/accept
 *     Body: { suggestionIds: uuid[] }
 *
 * Atomic transaction via the `accept_stakeholder_proposals_bulk` RPC:
 * creates stakeholders (+ resources where the reviewer toggled
 * `create_resource`), links duplicates (`stakeholder_link`) instead of
 * creating, writes `ki_provenance`, flips suggestions to accepted.
 * Returns the id arrays + `accepted_at` the FE needs for the 30-s Undo.
 *
...
export async function POST(request: Request, ctx: Ctx) {
  ...
  const { data, error } = await supabase.rpc(
    "accept_stakeholder_proposals_bulk",
    {
  ...
  return NextResponse.json(
    {
      ...
      created_stakeholder_ids: row.created_stakeholder_ids ?? [],
      created_resource_ids: row.created_resource_ids ?? [],
      linked_stakeholder_ids: row.linked_stakeholder_ids ?? [],
