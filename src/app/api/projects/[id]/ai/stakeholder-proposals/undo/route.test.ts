/**
 * PROJ-88 — vitest for the stakeholder-proposals undo route.
 *
...
function makePost(body: unknown): Request {
  return new Request(
    `http://localhost/api/projects/${PROJECT_ID}/ai/stakeholder-proposals/undo`,
    {
      method: "POST",
...
describe("POST /api/projects/[id]/ai/stakeholder-proposals/undo", () => {
  ...
  it("returns 200 + reverted ids on RPC success", async () => {
      ...
      data: [
        {
          ...
          reverted_stakeholder_ids: ["sh-1"],
          reverted_resource_ids: ["res-1"],
        },
      ],
      error: null,
    })
    ...
    expect(body.reverted_stakeholder_ids).toEqual(["sh-1"])
    expect(body.reverted_resource_ids).toEqual(["res-1"])
    expect(rpcMock).toHaveBeenCalledWith("accept_stakeholder_proposals_undo", {
  ...
  it("returns 400 on RPC undo_window_expired (22023)", async () => {
    ...
    rpcMock.mockResolvedValue({
      data: null,
      error: { code: "22023", message: "undo_window_expired" },
    })
    const res = await POST(makePost({ suggestionIds: [SUG_1] }), {
      params: Promise.resolve({ id: PROJECT_ID }),
    })
