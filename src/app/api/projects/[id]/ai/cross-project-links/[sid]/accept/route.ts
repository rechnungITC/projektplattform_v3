/**
 * PROJ-65 ε.4.γ — accept (advisory) a cross-project-link suggestion.
 *
 * POST /api/projects/[id]/ai/cross-project-links/[sid]/accept
 *
 * Flips `draft` → `accepted` without creating a downstream DB entity.
 * The actual `work_item_links` row is created via PROJ-27's existing
 * link-create dialog with its own audit trail — the AI accept is
 * intentionally advisory so the user keeps control over the final
 * link (link_type override, lag_days, approval flow).
 *
 * The relaxed `ki_suggestions_accepted_consistency` CHECK (PROJ-65
 * ε.4.γ migration) admits accepted-without-entity-link for this purpose.
 */
...
export async function POST(_request: Request, ctx: Ctx) {
  ...
  if (existing.purpose !== "cross_project_links") {
    return apiError(
      "wrong_purpose",
      "This endpoint only accepts cross_project_links suggestions.",
      400,
    )
  }
