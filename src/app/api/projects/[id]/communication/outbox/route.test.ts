...
// PROJ-13 — collection endpoint tests for /api/projects/[id]/communication/outbox.
...
function makePost(body: unknown): Request {
  return new Request(
    `http://localhost/api/projects/${PROJECT_ID}/communication/outbox`,
    {
      method: "POST",
...
describe("POST /api/projects/[id]/communication/outbox", () => {
  ...
  it("creates outbox draft on valid input (201)", async () => {
    getUserMock.mockResolvedValue({ data: { user: { id: USER_ID } } })
    ...
    const json = (await res.json()) as { outbox: { id: string } }
    expect(json.outbox.id).toBe("o1")
  })

  it("returns 403 when caller is not editor/lead/admin", async () => {
    getUserMock.mockResolvedValue({ data: { user: { id: USER_ID } } })
    tenantMembershipChain.maybeSingle.mockResolvedValue({
      data: { role: "member" },
      error: null,
...
describe("GET /api/projects/[id]/communication/outbox", () => {
  it("returns 401 when not signed in", async () => {
    getUserMock.mockResolvedValue({ data: { user: null } })
    const res = await GET(
      new Request(
  ...
  it("returns 200 with outbox list", async () => {
    getUserMock.mockResolvedValue({ data: { user: { id: USER_ID } } })
    listChain.__result = {
      data: [
        {
    ...
    const json = (await res.json()) as { outbox: unknown[] }
    expect(json.outbox).toHaveLength(1)
  })

  it("filters by channel + status when query params valid", async () => {
    getUserMock.mockResolvedValue({ data: { user: { id: USER_ID } } })
    listChain.__result = { data: [], error: null }
    const res = await GET(
      new Request(
        `http://localhost/api/projects/${PROJECT_ID}/communication/outbox?channel=email&status=sent`
      ),
      { params: Promise.resolve({ id: PROJECT_ID }) }
    )
    expect(res.status).toBe(200)
    expect(listChain.eq).toHaveBeenCalledWith("channel", "email")
