...
// PROJ-15 — vendor evaluations POST drift-detection.
...
const fromMock = vi.fn((table: string) => {
  ...
  if (table === "vendor_evaluations") return evalsChain
...
beforeEach(() => {
  ...
  evalsChain.insert.mockReturnValue(evalsChain)
  evalsChain.select.mockReturnValue(evalsChain)
...
describe("POST /api/vendors/[vid]/evaluations — schema/DB-payload drift", () => {
  it("forwards every Zod-schema field to the DB insert payload", async () => {
    const { evaluationCreateSchema } = await import("./_schema")
    getUserMock.mockResolvedValue({ data: { user: { id: USER_ID } } })

    const kitchenSink = {
      criterion: "Liefertreue",
      score: 4,
      comment: "Drift-Test comment",
    }

    const schemaKeys = Object.keys(evaluationCreateSchema.shape)
    ...
    evalsChain.single.mockResolvedValue({
      data: { id: "drift-1", ...kitchenSink, tenant_id: TENANT_ID },
      error: null,
    })

    const res = await POST(
      new Request(`http://localhost/api/vendors/${VENDOR_ID}/evaluations`, {
        method: "POST",
    ...
    const arg = evalsChain.insert.mock.calls[0]?.[0] as Record<string, unknown>
