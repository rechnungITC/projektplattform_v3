...
// `X-If-Unmodified-Since` exactly when the caller passes it. Without the
...
describe("updateResource — X-If-Unmodified-Since", () => {
  ...
  it("sends X-If-Unmodified-Since when ifUnmodifiedSince is provided", async () => {
    ...
    expect(headers["X-If-Unmodified-Since"]).toBe("2026-05-08T14:00:00.000Z")
    expect(headers["Content-Type"]).toBe("application/json")
  })

  it("omits X-If-Unmodified-Since when no token is provided", async () => {
    await updateResource(RESOURCE_ID, { display_name: "Renamed" })

    const init = fetchSpy.mock.calls[0]?.[1] as RequestInit | undefined
    const headers = init?.headers as Record<string, string>
    expect(headers).not.toHaveProperty("X-If-Unmodified-Since")
  })
