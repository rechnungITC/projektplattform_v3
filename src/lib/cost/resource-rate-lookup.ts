 ...
 *   `_resolve_role_rate` lockdown migration `20260502023615`). Callers must
 ...
 *     Matches the engine's contract that a missing rate produces a
 *     placeholder cost-line with a `no_rate_for_role` warning rather than
 *     blocking the allocation write (PROJ-24 Tech Design §12).
...
export async function resolveResourceRates(
  ...
  const settled = await Promise.all(
    uniqueKeys.map(async (key) => {
      ...
      if (error) {
        // Fail-open: log + treat as missing. Do not throw.
        console.error(
      ...
      // Normalize resource_id to the input key's resource_id (the SQL helper
      // returns NULL for it on the role-fallback branch).
 ...
 * means "no row" (caller treats as missing).
 */
