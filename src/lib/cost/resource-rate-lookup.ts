 ...
 *   `_resolve_role_rate` lockdown). Callers must use `createAdminClient()`
 *   from `@/lib/supabase/admin`. This module does NOT verify the supplied
 *   client carries service-role headers — the constraint is documented at
 *   the JSDoc level.
 *
 * Behavior:
 *   - Duplicate keys are deduplicated before RPC dispatch.
 *   - Per-key RPC failures are FAIL-OPEN: the failing key lands in
 *     `missing[]`, not `resolved[]`, and the lookup does NOT throw.
