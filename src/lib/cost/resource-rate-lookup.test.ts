...
function roleRow(
  ...
  return {
    ...
    source: "role",
    role_key: roleKey,
    // SQL helper sets resource_id to NULL on the role branch — the lookup
    // layer normalizes it to the input key's resource_id (T2 invariant).
    resource_id: null,
    daily_rate: daily,
    currency,
...
describe("resolveResourceRates", () => {
  ...
  it("T2 — Kein Override + Rolle aktiv: ResolvedRate.source === 'role' und resource_id wird vom Lookup-Layer injiziert", async () => {
    ...
    // Critical invariant: lookup layer rewrites resource_id from the input
    // key, even when the SQL helper returned NULL on the role branch.
    expect(r.resource_id).toBe(RES_A)
  })

  it("T3 — Override und Rolle gleichzeitig: Override gewinnt (Server-Side short-circuit; Lookup-Layer transportiert die override-Antwort 1:1)", async () => {
    // Mirrors the actual server-side semantics: when an override exists,
    // _resolve_resource_rate returns ONLY the override row and never executes
    // the role branch.
    const { client } = makeMockClient(() => ({
      data: [overrideRow(RES_A, 1500, "EUR")],
      error: null,
  ...
  it("resolves multiple distinct keys in parallel; resource_id-injection is consistent across branches", async () => {
  ...
  it("rejects malformed RPC responses (missing source field) as missing", async () => {
    const { client } = makeMockClient(() => ({
      data: [
        {
          tenant_id: TNT,
          // source missing → invalid payload
          role_key: "senior_dev",
          resource_id: null,
          daily_rate: 1000,
          currency: "EUR",
