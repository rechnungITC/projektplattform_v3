 ...
 *
 * Five isolated scenarios per AC-20:
...
function roleRow(
  ...
  return {
    ...
    source: "role",
    role_key: roleKey,
    // SQL helper sets resource_id to NULL on role branch; lookup-layer
    // normalizes it to the input key's resource_id (T2 invariant).
    resource_id: null,
    daily_rate: daily,
    currency,
...
describe("resolveResourceRates", () => {
  ...
  it("T3 — Override und Rolle gleichzeitig: Override gewinnt (server-side short-circuit)", async () => {
    // Mirrors actual server-side semantics: when an override exists, the
    // helper returns ONLY the override row and never executes the role branch.
    const { client } = makeMockClient(() => ({
      data: [overrideRow(RES_A, 1500, "EUR")],
      error: null,
    }))
