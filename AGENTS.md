# Projektplattform V3 — agent instructions

> **This file is kept in sync with `CLAUDE.md`.** Same content, minus Claude-Code-specific syntax.
...
Work moves through six stages. In Claude Code each is a slash command backed by a skill in
...
`CLAUDE.md` pulls these in automatically; other tools must open them explicitly. Read both before
planning any change — they are the product and status source of truth:
...
This project is indexed by GitNexus as **projektplattform_v3** (96461 symbols, 194363 relationships, 300 execution flows). Use the GitNexus MCP tools to understand code, assess impact, and navigate safely.
