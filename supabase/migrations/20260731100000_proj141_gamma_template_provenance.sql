-- PROJ-141-γ3 — Template-Provenance härten (M-3 aus Cross-cutting-Audit).
--
-- Problem: `workstreams.source_template_id` und `deliverables.source_template_id`
-- stehen auf ON DELETE SET NULL. Nach Template-Delete verliert das Projekt jede
-- Provenance-Identität, obwohl `source_template_version` als isoliertes Feld
-- stehen bleibt — man weiß nur noch „irgendeine Version irgendeines Templates".
--
-- γ3 Lösung (Option A — RESTRICT + Text-Snapshot, User-locked):
...
--      wechseln von SET NULL → RESTRICT (aligned enforcement).
...
--      da wir den Zeitpunkt nicht rekonstruieren können).
--
