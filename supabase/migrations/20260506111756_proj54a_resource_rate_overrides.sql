...
--   `linked_stakeholder_id`, `notes`) that no longer exist on the `resources`
--   table. The `record_audit_changes` trigger silently no-ops on missing
--   columns (uses `to_jsonb(NEW) -> v_col`), so this is not a data-integrity
--   bug — but it means almost no resource edits currently produce audit
--   entries. Cleaning that up is out of scope for PROJ-54-α (separate
--   hygiene-Slice candidate). For α we ONLY APPEND the two new override
--   columns; the stale entries continue to silently no-op.
-- =============================================================================
