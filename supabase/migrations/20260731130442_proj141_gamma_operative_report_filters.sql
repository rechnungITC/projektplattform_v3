-- PROJ-141-γ4/γ5-supplement — operative_report widened with optional filter args.
--
-- Historie: PROJ-141-γ shippte im Merge #288 die Konsistenz-Fixes M-4/M-5/M-6
-- clientseitig in `operative-report-view.tsx` (Findings/Q&A werden nach
-- dd_stream vor-gefiltert, Pre-Read wird neu berechnet, hasRows-Flags pro
-- Sektion; Export erhielt ein ehrliches „Gesamtreport"-Relabel — Filter-
-- Propagation an CSV/Print wurde bewusst nach PROJ-Y-132-filtered-export
-- deferred). Diese Supplement-Migration wurde in derselben γ-Session parallel
-- gegen Prod appliziert (via MCP `apply_migration`, Prod-Version
-- 20260731130442) und verbreitert die RPC-Signatur um vier optionale Filter-
-- Args, die bei künftiger Umsetzung von PROJ-Y-132-filtered-export das
-- server-seitige Filtern in Pre-Read + CSV + Print konsistent machen.
--
-- Aktueller Konsumenten-Status: alle deployten Callers rufen `operative_report`
-- 1-arg (`p_project_id`) → alle vier neuen Args greifen ihren Default `null` →
-- byte-identisches Passthrough zur Pre-γ-Semantik. Die deployte PROJ-132-Live-
-- Pentest bleibt verbatim grün. Keine FE- oder Route-Änderung nötig, bis
-- PROJ-Y-132-filtered-export die Query-Param-Threading-Schicht liefert.
--
-- Filter semantics (siehe künftige PROJ-Y-132-Tech-Design):
--
