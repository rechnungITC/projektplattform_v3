-- ===========================================================================
-- PROJ-34 ε.α post-merge Audit-Whitelist Restore (F-9)
-- ===========================================================================
-- PROJ-61's `20260513150000_proj61_releases_backend` migration rewrote
-- `_tracked_audit_columns` and `audit_log_entity_type_check` after my
-- ε.α migration was applied to live, dropping the
-- `stakeholder_coaching_recommendations` entry from both whitelists.
-- Effect: field-level audit on coaching recommendations was silently
-- disabled — UPDATEs don't fire audit rows because the column whitelist
-- returns []::text[]. DSGVO Art. 17 cascade-delete via FK still works.
--
-- This migration takes PROJ-61's whitelist as the new canonical baseline
-- and re-adds the SCR entries. Same pattern as PROJ-34-γ.2 F-1.
-- ===========================================================================
...
-- ---------------------------------------------------------------------------
-- audit_log_entity_type_check — re-add SCR entity type (PROJ-61 dropped it)
-- ---------------------------------------------------------------------------
