-- =============================================================================
-- PROJ-70-delta QA-Fix — accept/undo RPC repair (H-1 + H-2)
-- =============================================================================
-- H-1 (HIGH): accept_proposal_from_context_bulk inserted
--   ki_provenance.entity_type = 'work_item' (singular) but the PROJ-12
--   CHECK constraint ki_provenance_entity_check only allows the plural
--   'work_items' (besides risks/decisions/open_items). Every live accept
--   since the beta deploy failed with 400. Found by the first real
--   end-to-end accept (delta AC-d9 E2E smoke); beta vitest mocked the DB.
-- H-2 (MEDIUM): accept_proposal_from_context_undo did not delete the
--   ki_provenance row; UNIQUE(ki_suggestion_id) then blocks re-accept
--   after undo.
-- This migration re-creates BOTH functions from the beta definition with
-- exactly those two changes. ki_suggestions.accepted_entity_type keeps the
-- singular 'work_item' (no enum CHECK; undo + FE reference it).
-- ============================================================================
...
begin
  ...
  -- H-2 fix (PROJ-70-delta QA 2026-06-07): remove the provenance rows of
  -- the reverted suggestions. ki_provenance has UNIQUE(ki_suggestion_id);
  -- without this delete a re-accept after undo violates that constraint.
...
-- NOTE: the beta migration declared this comment with signature
-- (uuid, uuid) — wrong arg types, which is why prod carries no comments.
-- Fixed to the real signature here.
