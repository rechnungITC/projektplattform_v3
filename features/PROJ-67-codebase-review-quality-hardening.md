...
## Status: In Progress (AC-1/2/3/6 closed 2026-06-11; AC-7 closed 2026-06-12 als dokumentierte Upstream-Deviation; AC-4 + AC-5 closed 2026-05-30; AC-8 closed 2026-05-31; nur AC-9 pending)

## Implementation Notes — F7 (2026-06-12)

- **F7/AC-7 — GitNexus Query ReadOnly-FTS:** Root-Cause ist ein **Upstream-Design-Bug in gitnexus 1.6.3**: `analyze` verschiebt den FTS-Index-Aufbau bewusst auf „lazy beim ersten Query" (`core/run-analyze.js` Phase 3), aber **jeder** Query-Pfad öffnet die LadybugDB hardcoded read-only (`core/lbug/pool-adapter.js`: „MCP server never writes"). Der lazy `CREATE_FTS_INDEX` kann also nie gelingen → Warning bei jedem Query + dauerhaft leere BM25-Suche.
  - **Workaround geliefert:** `scripts/gitnexus-build-fts.mjs` (+ `npm run gitnexus:fix-fts`) baut die 5 FTS-Indexe (File/Function/Class/Method/Interface, Definitionen gespiegelt aus `core/search/bm25-index.js`) mit einer eigenen Write-Connection; crash-feste synchrone Logs; optionaler Tabellen-Filter für Einzelindex-Builds; Lock-Hinweis. Muss nach jedem `gitnexus analyze` erneut laufen (analyze recreated die DB ohne FTS).
  - **Limitation (dokumentiert, nicht lösbar auf App-Ebene):** `CALL CREATE_FTS_INDEX` segfaultet auf diesem Index (22k Nodes, `content`-Properties, WSL2) **nicht-deterministisch im LadybugDB-Native-Modul** — in 6 Versuchen committete genau 1 Index (`file_fts`, ~15 min Laufzeit), die übrigen crashten teils nach Minuten in der CREATE-Phase (verifiziert via synchronem `writeSync`-Logging: kein „created" vor dem Crash). DB bleibt dabei lesbar/konsistent (WAL-Rollback), `analyze` stellt jederzeit einen sauberen Zustand her.
  - **Was funktioniert:** `gitnexus context`/`impact`/`detect-changes`/`cypher` (Symbol-Lookup ohne FTS) liefern korrekte Ergebnisse — die CLAUDE.md-Pflicht-Workflows (Impact vor Edit, detect-changes vor Commit) sind **nicht** beeinträchtigt. Nur die BM25-Konzeptsuche (`query`) bleibt leer; Vector-Phase liefert mangels Embedding-Property ebenfalls keine Treffer (separates Upstream-Thema).
  - **Empfehlung/Handoff:** Upstream-Issue bei gitnexus einreichen (FTS-Build zurück in `analyze` verlagern — der eigene Code-Kommentar nennt das als ursprüngliches Design — plus LadybugDB-Segfault-Report). Bis dahin: `npx gitnexus query` nicht als verlässliche Konzeptsuche verwenden; `context`/`impact` decken die Pflicht-Workflows ab. AC-7 im Wortlaut („liefert wieder Prozess-/Symboltreffer") ist damit **nicht voll erfüllt** — geschlossen als Deviation über den im Implementation-Plan vorgesehenen Pfad „beheben **oder** upstream/workaround dokumentieren" (Item 7).
...
- [x] AC-7: `gitnexus query` funktioniert ohne ReadOnly-FTS-Warnungen und liefert wieder Prozess-/Symboltreffer. **Geschlossen 2026-06-12 als dokumentierte Deviation** — Upstream-Bug gitnexus 1.6.3 (lazy-FTS vs. read-only-Pool) + LadybugDB-Native-Segfault beim Index-Build; Workaround-Skript `npm run gitnexus:fix-fts` geliefert; `context`/`impact`/`detect-changes` (Pflicht-Workflows) verifiziert funktionsfähig. Details in Implementation Notes F7.
