...
- ~~**PROJ-Y-142b** — Audit der übrigen mock-only Parser-Abdeckung (`mammoth`/DOCX, `mailparser`/EML, `@kenjiuno/msgreader`/MSG).~~ **Erledigt 2026-08-10** → [PROJ-Y-142b](PROJ-Y-142b-real-parser-coverage.md). Scope-Korrektur dabei: `mailparser` war entgegen dieser Notiz **bereits** real getestet; offen waren `mammoth`, `msgreader` und zusätzlich der `file-type`-**Dispatch**. Die neue Abdeckung deckte einen echten Fehler auf (`sniffMagic` schnitt den Buffer auf 4100 Bytes und wies gültige `.docx` mit 415 ab) — behoben und red-green gepinnt.
