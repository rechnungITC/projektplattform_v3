...
| PROJ-54 | Resource-Level Tagessatz-Zuweisung — 2 Override-Spalten auf `resources` (Latest-only), neuer `_resolve_resource_rate`-Helper (Override → Rolle → null), `ResolvedRate`-Type mit Quellen-Feld, shadcn-Combobox (Rolle-Suche + Inline-Override) im Resource-Form, Bestand-Banner, Optimistic-Lock, async Recompute via Next.js `after()` mit Failed-Marker. Tenant-Admin-only. CIA-reviewed; α/β/γ-Slices, δ deferred. | In Progress (54-α + 54-β implemented inkl. Optimistic-Lock + Listen-Spalte + 21/21 Tests; Recompute γ pending) | [Spec](PROJ-54-resource-day-rate-assignment.md) | 2026-05-06 |
