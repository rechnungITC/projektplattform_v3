...
| PROJ-43 | Stakeholder-Health Critical-Path Detection — Korrektheits- und Coverage-Fix. α: `responsible_user_id`-Pfad + `linked_user_id`-only-Resources + Projekt-Filter (must-have, kein Schema-Change). β: `sprints.is_critical` + Method-Gating für Scrum (should-have). γ: zweiter computed-Flag aus `compute_critical_path_phases` ohne Trigger (deferred). CIA-reviewed. | Deployed (α + β live 2026-05-06) + Approved (γ backend + frontend: RPC-Aggregation + Sources-Tracking + Edit-Phase-Badge + Health-Tooltip-Differenzierung; QA passed 2026-05-06, awaiting /deploy) | [Spec](PROJ-43-stakeholder-critical-path-detection-fix.md) | 2026-05-05 |
