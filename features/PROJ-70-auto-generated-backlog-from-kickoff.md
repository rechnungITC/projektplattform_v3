...
## Status: Deployed (2026-07-23 Closure — Tag `v2.20.0-PROJ-70`; alle 5 Slices inkrementell live seit α 2026-06-01 bis ε 2026-06-08, Prod-Smoke POST /api/projects/[id]/ai/proposal-from-context → 307 auth-gate; reine Status-Closure des zuvor nur pro-Slice deployten Flagships) · α Approved+Deployed · β Approved+Deployed · γ Approved+Deployed (QA-Pass 2026-06-06: 14/16 AC fully PASS + 2 documented deviations F-1 Medium F-2 LOW; 11/12 security probes blocked; vitest 1654/1654; Playwright 16/16; 0 Critical/0 High → PRODUCTION-READY) · δ **Approved (QA-Pass 2026-06-07: 15/15 AC PASS + 1 documented deviation D-1; H-1/H-2/H-3 in-QA gefixt — β-Accept-RPC war in Prod doppelt kaputt (provenance-CHECK + unkorrelierter Toposort-Subquery), erster echter E2E-Accept überhaupt; vitest 1712/1712; Playwright 22 passed/2 skipped; 0 offene Critical/High → PRODUCTION-READY)** · ε **Approved (QA-Pass 2026-06-08: 8/8 AC PASS inkl. AC-ε7 Live-E2E + AC-ε2-Deviation; 1 LOW-Finding F-ε1 single-root-Tree-Viewport; vitest 1736/1736; PROJ-70 Playwright 25 passed/5 skipped serial; 0 Critical/0 High → PRODUCTION-READY)** — **PROJ-70 KOMPLETT über alle 5 Slices**

## Deployment Scope: full

> **Scope-Klassifikation (PROJ-Y-145b, Tranche 5, 2026-08-24):** Fuenf Teil-Slices (α/β/γ/δ/ε), **jede mit eigenem QA-Durchgang** und jede mit 0 Critical/0 High; die Spec schliesst mit „PROJ-70 KOMPLETT ueber alle 5 Slices". Der δ-Durchgang ist der wertvollste Beleg der ganzen Familie: der erste echte Ende-zu-Ende-Accept deckte **drei** Fehler in der β-RPC auf (Singular/Plural im `ki_provenance`-CHECK, unkorrelierte Toposort-Unterabfrage, Undo liess eine Provenienz-Zeile zurueck) — alle drei in der QA behoben, per Fix-Migration in Prod und live re-verifiziert. Offen sind ausschliesslich Lows ohne AC-Bezug (Tree-Viewport-Hoehe, LIST_SELECT-Spalten, RFC-4122-Strenge), von denen zwei ohnehin in ε geschlossen wurden.
...
- [x] **AC-δH-1** (MEDIUM, gegen Multipart-DoS): `simpleParser` mit `{ maxHtmlLengthToParse: 2_000_000, skipImageLinks: true }` aufrufen — 2-MB-Cap spiegelt den γ-raw-text-cap und deckelt die teuerste Operation. *(δ-backend: `eml-parser.ts` `MAX_HTML_LENGTH_TO_PARSE`)*
- [x] **AC-δH-2** (MEDIUM, gegen Multipart-DoS / mailparser Issue #202): Multipart-Bomben-Guard — nach Parse Gesamt-Part-/Attachment-Count gegen Limit **max. 50 Parts** prüfen; bei Überschreitung `FileParseError('email_too_many_parts')`. 20s-Timeout bleibt Backstop. *(δ-backend: `MAX_EMAIL_PARTS=50` in beiden Parsern + 51-Parts-Bomb-Test)*
- [x] **AC-δH-3** (MEDIUM, gegen Attachment-Buffering): **Attachments werden ignoriert, nicht extrahiert.** mailparser: `parsed.attachments` nie persistiert. msgreader: `getAttachment()` wird **nie** aufgerufen. Explizit als Code-Kommentar + Test dokumentiert (schließt `.msg`-in-`.msg`-Rekursion aus). *(δ-backend: Test "NEVER calls getAttachment" + Kommentare in beiden Parsern)*
- [x] **AC-δH-4** (LOW, gegen CFB-Malformed-Input): msgreader-Aufruf in try/catch → `FileParseError('msg_parse_failed')`; CFB-Magic-Byte-Check (`D0 CF 11 E0`) VOR Parser-Aufruf (γ-AC); `.eml` analog via Content-Type-Allowlist. *(δ-backend: deckt beide msgreader-Fehlersignale ab — throw UND `{dataType:null,error}`-Result)*
- [x] **AC-δH-5** (LOW): Excerpt-Extraktion priorisiert `parsed.text` (Plaintext) vor `parsed.html`; HTML→Text nur falls Plaintext leer. *(δ-backend: eml via mailparser-`text`; msg via `body`-vor-`bodyHtml` + `stripHtmlTags`-Fallback)*
- [x] **AC-δH-6** (LOW): Beide Parser via dynamic-import (γ-AC-8) innerhalb des bestehenden `Promise.race`-20s-Timeouts. Keine Änderung am γ-Orchestrator. *(δ-backend: `parseFile` dispatcht via `await import("./eml-parser" | "./msg-parser")` — Modul-Graph bleibt zyklenfrei)*
...
#### δ-Backend Implementation Notes — 2026-06-06

**Gebaut (Branch `proj-70/delta-backend`):**

- **Deps (pinned):** `mailparser@3.9.9` + `@kenjiuno/msgreader@1.28.0` (+ `@types/mailparser` dev). `npm audit`: nur die 2 pre-existing postcss/next-Moderates (→ PROJ-74), 0 aus den neuen Deps.
- **`src/lib/context-ingestion/eml-parser.ts`** — `parseEml` via mailparser `simpleParser` mit AC-δH-1-Optionen; `EmailMetadata` (subject/from/to/cc/date/message_id/in_reply_to/references/format) exakt nach δ-Data-Model; `normalizeAddressList` für single/array AddressObjects; Caps (25 MB size, 2 MB raw-text, 8000-char excerpt) gespiegelt von γ.
- **`src/lib/context-ingestion/msg-parser.ts`** — `parseMsg` via msgreader (CJS-double-default-Interop `mod.default.default`); AC-δH-4 deckt **beide** Fehlersignale (throw + `{dataType:null,error}`-Result — msgreader wirft bei malformed CFB nicht!); to/cc-Split über `recipType`; Threading-Header per Regex aus dem Transport-Header-Blob (`extractThreadingHeaders`, RFC822-Unfolding); `stripHtmlTags`-Fallback nur wenn Plain-Body leer.
- **`file-parser.ts` Dispatch** — `ALLOWED_MIME_TYPES` + `SupportedMime` + 2 neue `FileParseError`-Codes (`email_too_many_parts`, `msg_parse_failed`); `sniffMagic`: `message/rfc822`-Hint trusted (textbasiert, kein Magic-Byte), detected `application/x-cfb` → `application/vnd.ms-outlook` (Non-Outlook-CFB wie .doc/.xls fällt in parseMsg mit `msg_parse_failed` durch — dokumentiert); `parseFile` dispatcht via dynamic sibling-imports (zyklenfrei, AC-δH-6); `ParseResult.email?` optional für Email-Formate.
- **`/api/context-sources/route.ts`** — `inferMimeFromFilename`-Fallback (Browser senden für .eml/.msg oft leeren `File.type`); statusByCode-Mapping beide neuen Codes → 422; `source_metadata.proj70_delta_email` Merge (Lock-5).
- **Migration `20260611100000_proj70_delta_bucket_email_mimes.sql`** — Bucket-Allowlist + `message/rfc822` + `application/vnd.ms-outlook` (tolerantes DO-Block-Pattern wie γ, shadow-DB-safe). **In Prod-DB applied + verifiziert** (Allowlist = 6 MIMEs).
- **FE-Minimal-Touch:** File-Picker `accept` + Label + Kind-Inference `.eml/.msg → "email"` in `backlog-proposal-tab.tsx`. (Das eigentliche DnD-Reparenting AC-δ4..δ9 bleibt beim `/frontend`-Lauf.)
- **Tests: +36 neue Vitest-Cases** — `eml-parser.test.ts` (echter mailparser auf Inline-RFC822: Extraktion, Threading, HTML-Strip, 51-Parts-Bombe, Attachment-Leak-Check, Caps), `msg-parser.test.ts` (msgreader gemockt: Field-Mapping, beide Malformed-Signale, getAttachment-NEVER-called, Body-Precedence, Header-Unfolding), `file-parser.test.ts` (+5 Dispatch-Cases inkl. echtem msgreader-Garbage-Buffer → `msg_parse_failed`).
...
#### ε-Frontend Implementation Notes — 2026-06-08

**Gebaut (Branch `proj-70/epsilon-frontend`):**

- **`src/types/wizard.ts`** — `WIZARD_STEPS` um `ki_backlog` erweitert (zwischen `followups` und `review`) + Label; neuer `visibleWizardSteps(enabled)`-Helper (pure, testbar) liefert die aktive Step-Folge; `KiBacklogData`-Type (`enabled`/`context_source_id`/`filename`) + `emptyKiBacklogData()` in `WizardData`/`emptyWizardData`.
- **`wizard-client.tsx`** — `useWatch` (PROJ-67-AC-4-Konvention, memoisation-safe) auf `ki_backlog.enabled` → `steps`-Memo; gesamte Navigation (`goToStep`/Back/Next/Header/Stepper) läuft jetzt über `steps` statt der fixen `WIZARD_STEPS`; `clampedFurthest` fängt Toggle-Off-nach-Besuch ab; `validateStep` `case "ki_backlog"` immer valide (optional). **Finalize-Redirect**: bei aktivem ki_backlog + vorhandener `context_source_id` → `/projects/{id}/graph?mode=trajectory&aiDrawer=backlog&contextSource={id}` statt `?from_wizard=1` (Lock-Q1).
- **`step-basics.tsx`** — Switch-Toggle „KI-Backlog aus Kickoff-Datei generieren" (`ki_backlog.enabled`, AC-ε5).
- **`step-ki-backlog.tsx`** (neu) — Upload-Zone (alle δ-Formate via `uploadContextSourceFile` OHNE `project_id`), **sofortiger Upload beim Dateiwählen mit Parse-Fehler-Inline-Retry** (Lock-Q3), Skip ist implizit (Step blockt „Weiter" nie), Class-3-Hinweis; schreibt `context_source_id`+`filename` in den Draft-JSON-Block; `useWatch` statt `form.watch`.
- **`step-review.tsx`** — KI-Backlog-Karte zeigt Dateiname + „wird nach dem Anlegen generiert" bzw. „übersprungen" (AC-ε2-Sichtbarkeit).
- **`wizard-stepper.tsx`** — optionaler `steps`-Prop (Default = voller Katalog), rendert gegen die aktive Folge.
- **Deep-Link-Drawer:** `graph-shell.tsx` erzwingt `mode=trajectory` bei `?aiDrawer=…`; `ai-proposal-drawer.tsx` neuer `defaultTab` + `autoGenerateContextSourceId`-Prop (Tabs `defaultValue={defaultTab}`); `backlog-proposal-tab.tsx` one-shot Auto-Generierungs-Effekt (Ref-guarded) für die übergebene Quelle; `trajectory-graph-view.tsx` liest `aiDrawer`+`contextSource` via `useSearchParams` einmalig auf Mount → öffnet Drawer auf Backlog-Tab + reicht `projectMethod` aus dem Snapshot durch; Reset von Tab/Auto-Source beim Schließen.
- **Draft-Persistenz:** kein Schema-Change — `wizardDataSchema` ist `.passthrough()`, `ki_backlog` zusätzlich explizit als optionales Objekt deklariert (validiert die Shape, die die Finalize-Route lesen wird).
- **Tests:** `src/types/wizard.test.ts` (+7 Cases: visibleWizardSteps-Reihenfolge beide Modi, emptyData-Defaults, JSON-Roundtrip = AC-ε6); 2 wizard-drafts-Drift-Tests um `ki_backlog`-Kitchen-Sink-Key ergänzt.
- **Live-Smoke:** authentifizierter Browser-Check bestätigt 5 Steps (Toggle aus) → 6 Steps mit „KI-Backlog" an Position 5 (Toggle an).
- **Quality-Gates:** lint **0/0** (useWatch löst die React-Compiler-Skip-Warnung), tsc clean (δ-Files), vitest **1722/1722**, build clean 15.7s.
