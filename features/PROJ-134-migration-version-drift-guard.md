...
**User-Handoff (F-1) — Ruleset-Enrollment eingelöst 2026-08-13 (PROJ-Y-145d, PR #370):** Der Context `Verify migration filename naming + version-prefix uniqueness` ist im Ruleset `main protection` (ID 15992143) als *Required* hinterlegt — vorher 4 Checks, jetzt 5. **AC-134.2 ist damit vollständig „Required"**; der Guard ist keine advisory-Warnung mehr, sondern eine echte Merge-Sperre. Der Handoff stand seit dem 2026-07-01 offen und wurde bei der Ruleset-Arbeit von PROJ-Y-145c wiedergefunden: die CLAUDE.md-Tabelle führte den Check die ganze Zeit unter „A PR cannot merge until all of these pass", tatsächlich war er nur beratend. Eingetragen auf ausdrücklichen Nutzer-Entscheid (Branch-Protection gehört dem Repo-Eigner), Name vorab gegen `check-runs` auf `main` verifiziert, Ruleset-Diff nach dem Schreiben erneut von der API gelesen (einzige Änderung: der eine Context; alle übrigen Regeln byte-identisch, `bypass_actors` leer).
