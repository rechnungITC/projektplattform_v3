...
- [x] AC-2: Override darf **nur von Tenant-Admins** gesetzt werden — wird im API-Layer (PATCH-Whitelist) durchgesetzt (Tech-Design Fork 1c-Default). RLS auf `resources` bleibt unverändert.
...
- [x] AC-7: Neuer SQL-Helper `_resolve_resource_rate(p_tenant_id, p_resource_id, p_as_of_date)` — `SECURITY DEFINER`, `set search_path = public, pg_temp` (PROJ-29-Pattern), EXECUTE nur für `service_role` (analog `_resolve_role_rate`-Lockdown).
...
- [x] AC-20: Vitest-Coverage in `resource-rate-lookup.test.ts` mit 5 Szenarien T1-T5 (Override-only, Role-only, Override-wins-server-side, Neither-fails, RPC-error-fail-open) + Bonus-Cases (Dedup, Parallel, malformed RPC).
...
**Migration:** `supabase/migrations/20260506111756_proj54a_resource_rate_overrides.sql`
- Zwei Override-Spalten auf `resources` (`daily_rate_override`, `daily_rate_override_currency`) + Consistency-CHECK + neuer `_resolve_resource_rate`-Helper + Audit-Whitelist-Erweiterung.
- Live appliziert per `mcp__supabase__apply_migration` auf Projekt `iqerihohwabyjzkpcujq`. Smoke-Tests:
  - Schema-Verify: beide Spalten present (numeric / character, nullable=YES, default=null).
  - Constraint-Verify: `0 + EUR` löst `check_violation` aus (live verifiziert).
  - Helper-Verify: existiert, `SECURITY DEFINER`, EXECUTE nur für `postgres`+`service_role` (live verifiziert).

**TS Code:**
- `src/lib/cost/types.ts` — neue Typen `ResolvedRate` und `ResourceRateLookupKey`.
- `src/lib/cost/resource-rate-lookup.ts` — neue Datei `resolveResourceRates()` mit FAIL-OPEN-Pattern (analog `resolveRoleRates`), Dedup, parallele RPCs, Normalisierung von `resource_id` auch für Role-Branch.
...
- `src/lib/cost/synthesize-cost-lines.ts` — Lookup-Wechsel: vorher `resolveRoleRates(distinctRoleKeys)`, jetzt `resolveResourceRates(distinctResourceIds)`. Engine bekommt `resolved_rates`, `role_rates: []`.
- `src/lib/cost/index.ts` — Exports für `resolveResourceRates`, `ResolvedRate`, `ResourceRateLookupKey`.

**Tests:**
- Neu: `src/lib/cost/resource-rate-lookup.test.ts` mit 9 Cases (5 AC-20-Szenarien + 4 Bonus: Dedup, Parallel, Empty-Short-circuit, Malformed-RPC).
- Update: `src/lib/cost/synthesize-cost-lines.test.ts` — RPC-Mock-Shape angepasst auf `_resolve_resource_rate` (TABLE-Shape mit `source`-Feld); 1 Test (`stakeholder without role_key`) explizit auf empty-array-Mock umgestellt um neue Helper-Semantik abzubilden.

**Bewusst NICHT angefasst:**
- `_resolve_role_rate` und `resolveRoleRates` bleiben für Backwards-Compat exportiert. Der bestehende PROJ-24-α-γ-Pfad ist unverändert nutzbar.
- Die V2-Stale-Einträge in `_tracked_audit_columns('resources')` (z.B. `name`, `role_key`, `linked_stakeholder_id` — nicht im aktuellen Schema) werden NICHT bereinigt. Hygiene-Slice-Kandidat.
- API-Layer-Whitelist (Override schreibbar nur für Tenant-Admin) folgt im 54-β-Slice mit der UI-Anbindung.

**Schema-Drift-Risiko (PROJ-42):** geprüft. Migration ist additiv (2 Spalten + 1 Helper); kein SELECT auf entfernte Spalten in `src/`. CI-Drift-Check trivial grün.

**Verifikation:**
- `npx vitest run src/lib/cost/` → **47/47 grün** (4 Test-Files, +9 neue Cases).
- `npx vitest run` (volle Suite) → **1130/1130 grün** über 126 Files (+9 vs. Pre-PROJ-54).
...
| AC | Beleg | Status |
|---|---|---|
| AC-1 (Override-Spalten) | Migration + Live-Schema-Verify | ✓ |
| AC-2 (Tenant-Admin-only via API-Layer) | Architektur-Entscheidung; UI-Anbindung in 54-β | ✓ (DB-Layer ready) |
| AC-3 (Audit-Whitelist) | `_tracked_audit_columns('resources')` enthält Override-Spalten | ✓ |
| AC-4 (Class-3-PII) | Comments im Migration-File | ✓ |
| AC-5 (Schema-Drift-CI grün) | Additiver Diff | ✓ |
| AC-6 (Auflösungsreihenfolge) | SQL-Helper kapselt Override→Role→NULL | ✓ |
| AC-7 (SQL-Helper Lockdown) | EXECUTE nur service_role; Live-Verify | ✓ |
| AC-8 (Cost-Engine-Aufrufer migriert) | synthesize-cost-lines.ts; Backwards-compat additive Engine-Param | ✓ |
| AC-20 (5 isolierte Tests) | resource-rate-lookup.test.ts T1–T5 | ✓ |

**Open für /qa:**
- Live-Smoke gegen Pilot-Tenant: Override setzen, Cost-Line erscheint mit `rate_source='override'` in source_metadata.
- Performance-Probe (statisch ist OK; Live-Bench in γ).
