...
- [x] **Modul `organization` deaktiviert**: 403 vor Upload-Stage. — War von Anfang an umgesetzt, aber nie abgehakt: `src/app/api/organization-imports/_helpers.ts:48-53` ruft `requireModuleActive(supabase, tenantId, "organization", { intent })` **vor** der Admin-Prüfung und vor jedem Datenzugriff, und alle fünf Import-Routen gehen durch diesen Helfer (`upload`/`commit`/`rollback` mit `write` → 403, `list`/`preview` mit `read` → 404). Nachgetragen 2026-08-19 im Rahmen von PROJ-Y-143n (AC-Y143n.6), das dabei den Fundort belegt hat: **dies war drei Monate lang die einzige Aufrufstelle des Schlüssels im ganzen Produktivcode** — PROJ-62 hatte seinen eigenen Teil des Tors nicht gebaut (Bug M2-Re), weshalb der Schalter nur halb wirkte. Regressionsdeckung dafür jetzt in `src/app/api/organization-imports/_helpers.test.ts` (7 Fälle; das Verzeichnis hatte vorher keinen einzigen Test). Ergänzung zur Formulierung: „403" trifft für die drei schreibenden Routen zu, die zwei lesenden antworten nach der Hausnorm PROJ-17 ST-02 mit 404.
